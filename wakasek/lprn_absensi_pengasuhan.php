...
// Query untuk mengambil data absensi pengasuhan
$query = "SELECT ap.*, u.nama_lengkap, u.username as nis, k.nama_kelas, 
          DATE_FORMAT(ap.tanggal, '%d/%m/%Y') as tgl
          FROM absensi_pengasuhan ap
          JOIN users u ON ap.user_id = u.id
          JOIN kelas k ON u.kelas_id = k.id
          WHERE ap.tanggal BETWEEN ? AND ?";
...
$query .= " ORDER BY ap.tanggal DESC, k.nama_kelas, u.nama_lengkap";
                                    ...
                                    <td>
                                        ...
                                        switch($absensi['status']) {
                                            case 'hadir':
                                                ...
                                                break;
                                            case 'hl':
                                                echo '<span class="badge badge-warning">Hadir Lambat</span>';
                                                break;
                                            case 'hn':
                                                echo '<span class="badge badge-info">Hadir Nol</span>';
                                                break;
                                            case 'bawabah':
                                                echo '<span class="badge badge-danger">Bawabah</span>';
                                                break;
                                        }
                                        ?>
                                    </td>
...
function filterData() {
    ...
    window.location.href = 'lprn_absensi_pengasuhan.php?start_date=' + start_date + 
                          '&end_date=' + end_date + 
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, `Laporan_Absensi_Pengasuhan_${start_date}_sd_${end_date}.xlsx`);
}
    ...
    doc.autoTable({
        ...
        didDrawPage: function(data) {
            ...
            if (data.pageNumber === 1) {
                ...
                doc.text('LAPORAN ABSENSI PENGASUHAN', doc.internal.pageSize.width/2, margin.top + 8, { align: 'center' });
