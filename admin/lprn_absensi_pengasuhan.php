...
// Tambahkan query untuk daftar kelas setelah koneksi database
...
$query = "SELECT u.id, u.nis, u.nama_lengkap, k.nama_kelas,
          COALESCE(MAX(ap.tanggal), ?) as tanggal,
          COALESCE(MAX(ap.status), '-') as status
          FROM users u 
          JOIN kelas k ON u.kelas_id = k.id
          LEFT JOIN absensi_pengasuhan ap ON u.id = ap.user_id 
            AND ap.tanggal BETWEEN ? AND ?
          WHERE u.role = 'siswa'
          $where_clause
          GROUP BY u.id, u.nis, u.nama_lengkap, k.nama_kelas, DATE(ap.tanggal)
          ORDER BY tanggal DESC, k.nama_kelas, u.nama_lengkap";
...
<div class="d-none d-print-block print-header">
    <h2>LAPORAN ABSENSI PENGASUHAN</h2>
...
function exportToExcel() {
    ...
    const wb = XLSX.utils.table_to_book(table, {sheet: "Absensi Pengasuhan"});
    const filename = `Laporan_Absensi_Pengasuhan_${document.querySelector('[name="start_date"]').value}_${document.querySelector('[name="end_date"]').value}.xlsx`;
...
function exportToPDF() {
    ...
    // Add title
    doc.setFontSize(16);
    doc.text('Laporan Absensi Pengasuhan', 14, 15);
    ...
    // Save PDF
    doc.save(`Laporan_Absensi_Pengasuhan_${document.querySelector('[name="start_date"]').value}_${document.querySelector('[name="end_date"]').value}.pdf`);
}
...
function printReport() {
    ...
    document.title = `Laporan Absensi Pengasuhan (${document.querySelector('[name="start_date"]').value} s/d ${document.querySelector('[name="end_date"]').value})`;
